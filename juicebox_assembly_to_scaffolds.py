...
class seq_range:
    ...
    def __str__(self):
        ...
        if self.is_zero_length:
            return ''
        if (self.type == 'U'):
            ret = 'PGA_scaffold_{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}'.format(self.sid, self.scaffold_offset, self.scaffold_offset + (agp_end - agp_start), self.part_number, self.type, 100,  'paired-ends')
        else:
            ret = 'PGA_scaffold_{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}'.format(self.sid, self.scaffold_offset, self.scaffold_offset + (agp_end - agp_start), self.part_number, self.type, self.name, agp_start, agp_end, self.strand)
        return ret
    ...
    def bed_line(self):
        ...
        if (self.type == 'U'):
            ret = 'PGA_scaffold_{0}\t{1}\t{2}\t{3}'.format(self.sid, self.scaffold_offset - 1, self.scaffold_offset + (self.end - self.start), 'pg_gap_{0}'.format(self.part_number/2))
        else:
            ret = 'PGA_scaffold_{0}\t{1}\t{2}\t{3}\t{4}\t{5}'.format(self.sid, self.scaffold_offset - 1, self.scaffold_offset + (self.end - self.start), self.name, 25,  self.strand)
        return ret
...
class scaffold:
    def __init__ (self, ind, header, sid):
        ...
        for contig_index in ind:
            ...
            pn += 1
            if (contig_index + 1) != abs(int(ind[-1])):  # here translate back to 1-index for one thing
    ...
    def dump_fasta(self, fasta, fasta_output_writer):
        ...
        fasta_output_writer.write('>PGA_scaffold{0}__{1}_contigs__length_{2}\n'.format(self.sid, (len(self.contigs) - self.gap_id) if len(self.contigs) > 1 else 1, total_scaffold_length))
...
bed_output_file = '{0}.bed'.format(args.prefix)
...
with open(args.assembly) as f:
    ...
    for items in sline:
        if items[0][0] != '>':
            scaffolds.append(scaffold(items, header, scaffold_id))
            scaffold_id += 1
        else:
